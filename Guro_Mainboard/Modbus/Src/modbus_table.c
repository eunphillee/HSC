...
static const PollEntry_t poll_table[POLL_TABLE_SIZE] = {
    ...
    { SLAVE_ID_HPSB, POLL_ENTRY_READ_INPUT_REG, MODBUS_INPUT_REG_START, 7 },
    /* LPSB1 */
    ...
    { SLAVE_ID_LPSB1, POLL_ENTRY_READ_INPUT_REG, MODBUS_INPUT_REG_START, 4 },
    /* LPSB2 */
    ...
    { SLAVE_ID_LPSB2, POLL_ENTRY_READ_INPUT_REG, MODBUS_INPUT_REG_START, 4 },
    /* LPSB3 */
    ...
    { SLAVE_ID_LPSB3, POLL_ENTRY_READ_INPUT_REG, MODBUS_INPUT_REG_START, 4 },
};

static inline uint8_t slave_idx(SlaveId_t slave)
{
    return (uint8_t)SLAVE_ID_TO_TABLE_INDEX(slave);
}
...
{
    if (!IS_VALID_SLAVE_ID(slave) || bit_index >= MODBUS_DISCRETE_COUNT)
        return 0;
...
{
    if (!IS_VALID_SLAVE_ID(slave) || bit_index >= MODBUS_COIL_COUNT)
        return 0;
...
{
    if (!IS_VALID_SLAVE_ID(slave) || reg_index >= MODBUS_HOLDING_COUNT)
        return 0;
...
{
    if (!IS_VALID_SLAVE_ID(slave) || bit_index >= MODBUS_DISCRETE_COUNT) return;
...
{
    if (!IS_VALID_SLAVE_ID(slave) || bit_index >= MODBUS_COIL_COUNT) return;
...
{
    if (!IS_VALID_SLAVE_ID(slave) || reg_index >= MODBUS_HOLDING_COUNT) return;
...
void ModbusTable_SetDiscreteBytes(SlaveId_t slave, const uint8_t *bytes, uint16_t num_bits)
{
    ...
    if (!IS_VALID_SLAVE_ID(slave) || bytes == NULL) return;
