...
static void process_frame(void)
{
  ...
  else if (fc == 0x04u) /* Read Input Registers (Direct LPSB용 최소 추가) */
  {
    uint16_t start = (uint16_t)((uint16_t)rx_buf[2] << 8) | rx_buf[3];
    uint16_t qty   = (uint16_t)((uint16_t)rx_buf[4] << 8) | rx_buf[5];
    /* reg0..9: DI image, AVG1..3, PKPK1..3, CUR1..3 */
    if (qty > 125u || start > 9u || (start + qty) > 10u)
    {
      send_exception(slave, fc, 0x02u);
      return;
    }
    uint16_t regs[10];
    uint16_t di_img = 0u;
    di_img |= (uint16_t)(LPSB_SSR_Get(0) ? 1u : 0u) << 0;
    di_img |= (uint16_t)(LPSB_SSR_Get(1) ? 1u : 0u) << 1;
    di_img |= (uint16_t)(LPSB_SSR_Get(2) ? 1u : 0u) << 2;
    regs[0] = di_img;
    regs[1] = LPSB_ADC_GetStoredAvg(0);
    regs[2] = LPSB_ADC_GetStoredAvg(1);
    regs[3] = LPSB_ADC_GetStoredAvg(2);
    regs[4] = LPSB_ADC_GetStoredPkpk(0);
    regs[5] = LPSB_ADC_GetStoredPkpk(1);
    regs[6] = LPSB_ADC_GetStoredPkpk(2);
    regs[7] = (uint16_t)(LPSB_SSR_Get(0) ? 1u : 0u);
    regs[8] = (uint16_t)(LPSB_SSR_Get(1) ? 1u : 0u);
    regs[9] = (uint16_t)(LPSB_SSR_Get(2) ? 1u : 0u);
